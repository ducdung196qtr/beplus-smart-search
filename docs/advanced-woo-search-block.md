...
> Primary feature spec for **BePlus Smart Search**. Defines the `advanced-woo-search` Gutenberg block: live WooCommerce product filtering **without page reload**, droppable into the shop template (`archive-product`).
...
|-------------|----------|
| Gutenberg block | `beplus-smart-search/advanced-woo-search` |
...
├── main
│   ├── beplus-smart-search/advanced-woo-search   ← NEW (drag here)
...
User changes filter (keyword / category / tag / attribute / stock)
        │
        ▼
JS debounce (keyword only) ──► GET /wp-json/beplus-smart-search/v1/products?...
        │
        ▼
...
| Item | Value |
|------|-------|
| Block name | `beplus-smart-search/advanced-woo-search` |
...
```html
<div class="beplus-smart-search beplus-smart-search--advanced-woo" data-bpss-advanced-woo-search>
  <form class="beplus-smart-search__form" role="search" method="get"
    ...
    <!-- Category -->
    <div class="beplus-smart-search__field beplus-smart-search__field--category">
      <label for="{id}-cat">Category</label>
      <select name="product_cat" id="{id}-cat" data-bpss-filter="category">…</select>
    </div>

    <!-- Tag -->
    <div class="beplus-smart-search__field beplus-smart-search__field--tag">
      <select name="product_tag" data-bpss-filter="tag">…</select>
    </div>

    <!-- Attributes (one select per pa_*) -->
    <div class="beplus-smart-search__field beplus-smart-search__field--attribute"
         data-attribute="{slug}">
    ...
    <button type="submit" class="beplus-smart-search__submit">Search</button>
    <button type="button" class="beplus-smart-search__clear" data-bpss-clear hidden>Clear</button>

    <span class="beplus-smart-search__status" role="status" aria-live="polite"
...
4. Escape all output; translatable strings with text domain `beplus-smart-search`.
...
// Toggle beplus-smart-search--loading on form
```
...
**Route:** `GET beplus-smart-search/v1/products`
...
```
GET /wp-json/beplus-smart-search/v1/products?s=beanie&product_cat=clothing&stock_status=instock&attribute[color]=blue&page=1&per_page=10
```
...
```php
apply_filters( 'beplus-smart-search/search.results', $items, $query );
do_action( 'beplus-smart-search/search.completed', $query, $items );
```

### 4.4 Facets endpoint (for editor + dynamic filters)

**Route:** `GET beplus-smart-search/v1/facets`
...
Cache with `wp_cache_set( 'bpss_facets', …, 'beplus_smart_search', HOUR_IN_SECONDS )`.
...
2. Apply `beplus-smart-search/search.query` filter.
...
- [ ] Plugin bootstrap (`beplus-smart-search.php`, `Plugin`, `Container`, `AbstractModule`)
...
- [ ] Filters: `beplus_smart_search.providers`, `beplus-smart-search/search.query`
...
```bash
curl "http://plugin.local/wp-json/beplus-smart-search/v1/products?s=beanie&per_page=5"
curl "http://plugin.local/wp-json/beplus-smart-search/v1/facets"
```
...
- Loading: `beplus-smart-search--loading` class; respect `prefers-reduced-motion`.
...
| AWS pattern | BePlus Smart Search approach |
|-------------|------------------------------|
| Custom DB index table | Phase 4 optional; MVP uses `wc_get_products` |
| AJAX admin-ajax | REST `beplus-smart-search/v1/products` |
