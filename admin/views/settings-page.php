...
$option_key            = BePlusFastProductFilterLiveSearch\Settings\SettingsRegistry::OPTION_KEY;
...
$attribute_definitions = function_exists( 'beplus_fast_product_filter_live_search_get_all_attribute_definitions' )
	? beplus_fast_product_filter_live_search_get_all_attribute_definitions()
	: array();
$brand_taxonomy        = function_exists( 'beplus_fast_product_filter_live_search_get_brand_taxonomy' )
	? beplus_fast_product_filter_live_search_get_brand_taxonomy()
	: 'product_brand';
$selectable_taxonomies = function_exists( 'beplus_fast_product_filter_live_search_get_selectable_product_taxonomies' )
	? beplus_fast_product_filter_live_search_get_selectable_product_taxonomies()
	: array();
$menu_slug             = BePlusFastProductFilterLiveSearch\Admin\SettingsPage::MENU_SLUG;
...
$cache_last_cleared    = BePlusFastProductFilterLiveSearch\Search\CacheService::get_last_cleared_timestamp();
$cache_ttl_presets     = BePlusFastProductFilterLiveSearch\Search\CacheService::get_ttl_presets();
$cache_item_catalog    = BePlusFastProductFilterLiveSearch\Search\CacheService::get_cached_item_catalog();
$cache_benchmark       = BePlusFastProductFilterLiveSearch\Search\CacheService::get_benchmark_stats();
...
<div class="wrap bpss-settings">
	...
	<form method="post" action="options.php" class="bpss-settings__form">
		<?php settings_fields( 'beplus_fast_product_filter_live_search' ); ?>
						...
						<select id="bpss-cache-ttl" name="<?php echo esc_attr( $option_key ); ?>[cache_ttl]">
							<?php foreach ( $cache_ttl_presets as $preset_minutes ) : ?>
								<option value="<?php echo esc_attr( (string) $preset_minutes ); ?>" <?php selected( $cache_ttl, $preset_minutes ); ?>>
									<?php echo esc_html( BePlusFastProductFilterLiveSearch\Search\CacheService::format_ttl_label( $preset_minutes ) ); ?>
								</option>
							<?php endforeach; ?>
						</select>
						<p class="description">
